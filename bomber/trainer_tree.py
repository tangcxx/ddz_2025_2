...
## 模型评价
nround_eval = 1200
models_op = [Model(), Model(), Model()]
cp_op = torch.load("model_op/cp101035.pt")
[models_op[pos].load_state_dict(cp_op["models_state_dict"][pos]) for pos in range(3)]
bots_op = [BOT(models=models_op), BOT(models=models_op), BOT(models=models_op)]
...
def eval(args):
    ...
    cards = arena.cards.copy()
    arena.registerbot([bots[0], bots_op[1], bots_op[2]])
    arena.wholegame()
    n_dizhu_win += (arena.winner == 0)
    
    arena2 = ARENA(verbos=0, cards=cards)
    arena2.registerbot([bots_op[0], bots[1], bots[2]])
    arena2.wholegame()
