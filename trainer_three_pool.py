# 基于trainer_three_3
# 不再传递weights, 改为load 模型
# 不再使用 epsilon, 采用前300个保存的模型来对局生成训练数据
# 测试应对可能的策略过拟合
...
def selfplay(iter):
    ids = range(0, iter+1, 50)[-300:]
    ids = np.random.choice(ids, 3)
    model_subs = [k.models.load_model(f"{modelpath}/{i}/m{id}.keras") for i, id in enumerate(ids)]
...
def train():
    ...
    while True:
        ...
        res = p.map(selfplay, [iter] * nmatch_per_iter)
        xss, yss = [[], [], []], [[], [], []]
